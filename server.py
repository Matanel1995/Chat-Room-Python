...
def handle(client):
    while True:
        try:
            ...
            message = message.decode('utf-8')
            temp = message.split(" ")
            # print the message on the sever log
            # print(temp[1] + '\n')
            # print(message + '\n')
            print(message)
            # If the user want to get all the users in chat room
            if temp[1] == 'get_users':
                file_list_message = '----- Users in chat room -------\n'
                for i in range(0, len(nicknames)):
                    file_list_message += '(' + str(i + 1) + ') ' + str(nicknames[i]) + '\n'
                file_list_message += '---------- That\'s all! ----------'
                file_list_message = file_list_message.encode('utf-8')
                client.send(file_list_message)
            # If the user want to disconnect
                ...
                thread.start()
                pass
            # If the user want to get the file list from the server
            elif temp[1] == 'get_list_file':
                file_list_message = '---Server File List---\n'
                for i in range(len(file_list)):
                    file_list_message += '(' + str(i + 1) + ') ' + str(file_list[i]) + '\n'
                file_list_message += '---End Sever File List---'
                file_list_message = file_list_message.encode('utf-8')
                client.send(file_list_message)
